...
from campaigns.ticai import models, const
from campaigns.ticai.applet.uitls import save_work_file
from campaigns.ticai.applet import decorators, QRCode, walkExcel, makeName
...
def priceInfo(request):
    try:
        ...
        hitPrizeinfo = models.walkCount.objects.filter(priCode__isnull=False).filter(creaTime__lte=endtime).filter(creaTime__gte=startime).all()[start: end]
        l1 = []
...
def ActiveData(request):
    try:
        ...
        total_count = models.walkCount.objects.filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).all().count()
        ...
        if request.GET['type'] == "1":
            ...
            activedata = models.walkCount.objects.all().filter(creaTime__lte=endtime).filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).\
            filter(creaTime__gte=statime).order_by("creaTime")[start: end]
            l1 = []
            for i in activedata:
                if i.image is None:
                    continue
    ...
    try:
        ...
        if usrid is not None:
            ...
            total_count = models.walkCount.objects.filter(openid=openid.openid, creaTime__gte=timestart,
                                                          creaTime__lte=timend).filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).all().count()
            ...
            if request.POST['type'] == "time" and request.POST.get("extype", None) != "1":
                userinfo = models.walkCount.objects.filter(openid=openid.openid, creaTime__gte=timestart,
                                                           creaTime__lte=timend).all().order_by('-creaTime').filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg"))[start: end]
                for i in userinfo:
                    d1 = {}
                ...
                userinfo = models.walkCount.objects.filter(openid=openid.openid, creaTime__gte=timestart,
                                                           creaTime__lte=timend).filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).all()
                for i in userinfo:
                    d1 = {}
                    d1['usrid'] = models.WXUser.objects.filter(openid=i.openid).first().id
                    d1['Cid'] = i.id
                ...
                l1 = []
                userinfo = models.walkCount.objects.filter(openid=openid.openid, creaTime__gte=timestart,
                                                           creaTime__lte=timend).filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).all().order_by('-change')[start: end]
                total_count = models.walkCount.objects.filter(openid=openid.openid, creaTime__gte=timestart,
                                                           creaTime__lte=timend).filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).all().count()
                for i in userinfo:
                    d1 = {}
            ...
            if request.POST['type'] == 'time' and request.POST['extype'] == None:
                userinfo = models.walkCount.objects.filter(creaTime__gte=timestart,
                                                           creaTime__lte=timend).filter(~Q(image="http://www.fantizi5.com/zi/kt/e58187.jpg")).all().order_by(
                    '-creaTime')[start: end]
                for i in userinfo:
                    d1 = {}
                    d1['usrid'] = models.WXUser.objects.filter(openid=i.openid).first().id
